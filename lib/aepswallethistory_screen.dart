...
import 'package:demo/Json_model/aepswallethistory_model.dart';
...
class Aepswallethistory_screen extends StatefulWidget {
  const Aepswallethistory_screen({Key? key}) : super(key: key);

  @override
  State<Aepswallethistory_screen> createState() =>
      _Rechargehistory_screenState();
}

class _Rechargehistory_screenState extends State<Aepswallethistory_screen> {
  ...
  List<aepswallethData> filteredItems = [];

  List<aepswallethData> aepswallethistoryArry = [];
  ...
  void filterItems(String query) {
    setState(() {
      filteredItems = aepswallethistoryArry
          .where((item) =>
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            if (fromDate.isNotEmpty && toDate.isNotEmpty) {
                              aepswallethistoryApi().then((value) {});
                            } else {
  ...
  Future<void> aepswallethistoryApi() async {
    ...
    String? Mobile = await SharedPreferenceUtils.getValue("mob");
      ...
      if (Mobile != null) {
        ...
        final historyreport = await MyGlobalFunctions.PostCall(
          '{"MethodName":"aepswallethistory","UserID":"$Mobile","Password":"$Password", "ToDate":"${toDateController.text}","Size":"${num}","Page":"${currentPage}","FromDate":"${fromDateController.text}"}',
          "$deviceID",
        );
        ...
        if (data["statuscode"] == "TXN") {
          ...
          final List<aepswallethData> serviceList =
              jsonList.map((json) => aepswallethData.fromJson(json)).toList();

          setState(() {
            aepswallethistoryArry.addAll(serviceList);
            filteredItems = List.from(aepswallethistoryArry);
